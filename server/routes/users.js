...
router.post('/login', asyncHandler(async(req, res) => {
  ...
  if (!email || !password) {
    ...
    if (isValid) 
      res.send({message: "Authorisation Successful"})
    else
...
router.put('/:userId/mark-favorite/:companyId', async(req, res) => {
...
router.put('/:userId/unmark-favorite/:companyId', async(req, res) => {
  const {userId, companyId} = req.params
  const user = await checkIfUserExists('_id', userId)
